...
linear1 = torch.nn.Linear(784, 128).cuda()
activ1 = torch.nn.Sigmoid().cuda()
linear = torch.nn.Linear(128, 10).cuda()

def model(DATA):
    ACTIVATION1 = linear1(DATA)
    ACTIVITY1 = activ1(ACTIVATION1)
    ACTIVATION = linear(ACTIVITY1)
    return ACTIVATION

batch = 100
optimizer = torch.optim.SGD([linear.bias, linear.weight,
                             linear1.bias, linear1.weight], lr = 0.1)
